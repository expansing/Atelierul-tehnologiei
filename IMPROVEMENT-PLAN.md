...
- [x] Verificare manuală completă a coerenței explicație -> demo -> exercițiu pentru toate lecțiile (toate cele 69, pe niveluri, via analiză dedicată per grup de niveluri). Singura problemă reală găsită: lecția 5.1 avea „8 etape” menționate în criteriile de trecere și în obiectivul proiectului, deși explicația/demo-ul/obiectivele arată consecvent 7 etape (tu → browser → DNS → router → internet → server → pagină) — corectat la „7 etape” în ambele locuri.
