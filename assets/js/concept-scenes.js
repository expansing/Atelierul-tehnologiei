...
const conceptMotionAllowed = typeof window === "undefined" || !(window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches);

function conceptValue(value, key, fallback = "") {
  return value && value[key] !== undefined && value[key] !== null ? value[key] : fallback;
}
function conceptPart(parts, index, key, fallback = "") {
  return conceptValue(parts[index], key, fallback);
}

function conceptVisualParts(object, fallback = "Idee") {
  const value = String(object || "");
  const iconMatch = value.match(/^(?:\p{Extended_Pictographic}|\p{Emoji_Presentation})/u);
  return {
...
function conceptPhaseCount(visual) {
  const kind = String(conceptValue(visual, "kind")).toLowerCase().replace(/[^a-z0-9]+/g, "-");
  const objects = conceptValue(visual, "objects");
  return ["binary", "layers", "timeline", "scratch"].includes(kind) ? 4 : (Array.isArray(objects) ? objects.length : 3);
}
...
function conceptSceneFrame(visual, kind, uid, body, descriptionId = "") {
  const titleId = `concept-title-${uid}`;
  const descriptionAttribute = descriptionId ? ` aria-describedby="${esc(descriptionId)}"` : "";
  return `<svg class="story-svg visual-svg visual-${esc(kind)}" viewBox="0 0 640 300" role="img" aria-labelledby="${titleId}"${descriptionAttribute}>
    <title id="${titleId}">${esc(conceptValue(visual, "title", "Scenă vizuală a lecției"))}</title>
...
function renderConceptScene(visual, stepIndex, stepCount, uid = `concept-${++conceptSceneSequence}`, descriptionId = "") {
  ...
  if (["objects", "invention", "computer", "algorithm"].includes(kind)) {
    ...
    body = `${conceptSceneConnection(125, 150, 255, 150)}${conceptSceneConnection(385, 150, 515, 150)}
      <g class="logic-switch ${aOn ? "is-closed" : ""}" transform="translate(100,150)"><circle class="scene-node-circle" r="30"></circle><path class="logic-lever" d="M -22 12 L 12 -12"></path><text class="scene-node-icon" y="9">${esc(conceptPart(parts, 0, "icon", "A"))}</text><text class="scene-node-label" y="57">A</text></g>
      <g class="logic-switch ${bOn ? "is-closed" : ""}" transform="translate(320,150)"><circle class="scene-node-circle" r="30"></circle><path class="logic-lever" d="M -22 12 L 12 -12"></path><text class="scene-node-icon" y="9">${esc(conceptPart(parts, 1, "icon", "B"))}</text><text class="scene-node-label" y="57">B</text></g>
    ...
    body = `<path class="circuit-wire" d="M 95 220 L 95 82 L 320 82 L 320 220 L 545 220 L 545 82 L 95 82"></path>
      <g class="circuit-battery" transform="translate(95,220)"><rect x="-30" y="-20" width="60" height="40" rx="8"></rect><text class="scene-panel-title" x="0" y="7">${esc(conceptPart(parts, 0, "label", "Baterie"))}</text></g>
      <g class="circuit-resistor ${active === 1 ? "is-active" : ""}" transform="translate(320,82)"><rect x="-48" y="-18" width="96" height="36" rx="8"></rect><text class="scene-panel-title" x="0" y="7">${esc(conceptPart(parts, 1, "label", "Rezistență"))}</text></g>
      <g class="circuit-led ${active === 2 ? "is-lit" : ""}" transform="translate(545,82)"><circle class="scene-node-circle" r="30"></circle><text class="scene-node-icon" y="9">💡</text><text class="scene-node-label" y="57">${esc(conceptPart(parts, 2, "label", "LED"))}</text></g>
    ...
    body = names.map((name, i) => `<g class="layer-block ${active === i ? "is-active" : ""}" transform="translate(${145 + i * 12}, ${58 + i * 48})"><rect width="350" height="38" rx="10"></rect><text class="scene-panel-title" x="175" y="25">${esc(conceptPart(parts, i, "label", name))}</text></g>${i < names.length - 1 ? `<path class="layer-arrow" d="M 320 ${112 + i * 48} L 320 ${136 + i * 48}"></path>` : ""}`).join("");
  } else if (kind === "terminal") {
    body = `<g class="terminal-window"><rect x="90" y="55" width="460" height="180" rx="16"></rect><circle cx="122" cy="82" r="6"></circle><circle cx="145" cy="82" r="6"></circle><text class="terminal-command" x="120" y="125"><tspan class="${active === 0 ? "is-active" : ""}">$ ${esc(conceptPart(parts, 0, "label", "comandă"))}</tspan></text><text class="terminal-output" x="120" y="160">${esc(conceptPart(parts, 2, "label", "rezultat"))}</text><text class="terminal-cursor" x="310" y="160">▌</text></g>`;
    ...
    body = `${conceptScenePanel(55, 70, 155, 160, conceptPart(parts, 0, "label", "Date"), "rânduri", active === 0)}${conceptSceneConnection(210, 150, 275, 150)}${conceptScenePanel(280, 70, 155, 160, conceptPart(parts, 1, "label", "Întrebare"), "SELECT", active === 1)}${conceptSceneConnection(435, 150, 485, 150)}${conceptScenePanel(490, 70, 105, 160, conceptPart(parts, 2, "label", "Rezultat"), "tabel", active === 2)}${conceptScenePacket("M 210 150 L 485 150")}`;
  } else if (kind === "cloud") {
    body = `${conceptSceneNode(100, 150, parts[0] || conceptVisualParts("💻 Tu"), active === 0)}${conceptSceneConnection(145, 150, 255, 115)}<g class="cloud-shape ${active === 1 ? "is-active" : ""}" transform="translate(320,115)"><path d="M -55 18 Q -55 -18 -20 -18 Q -8 -42 20 -28 Q 52 -31 52 0 Q 70 4 58 22 Q 48 38 15 30 Q -35 42 -55 18 Z"></path><text class="cloud-label" x="0" y="10">☁️</text></g>${conceptSceneConnection(385, 145, 495, 190)}${conceptScenePanel(485, 165, 110, 70, conceptPart(parts, 2, "label", "Centru"), "date", active === 2)}${conceptScenePacket("M 145 150 C 205 120, 255 120, 275 115 S 435 170, 495 190")}`;
    ...
    body = `${conceptScenePanel(55, 75, 145, 150, conceptPart(parts, 0, "label", "Exemple"), "date", active === 0)}${conceptSceneConnection(200, 150, 265, 150)}<g class="ai-brain ${active === 1 ? "is-active" : ""}" transform="translate(330,150)"><path d="M -38 -22 Q -58 8 -34 25 Q -43 48 -12 43 Q -3 64 20 47 Q 50 54 48 25 Q 68 5 45 -17 Q 30 -43 2 -32 Q -22 -45 -38 -22 Z"></path><text class="brain-label" x="0" y="8">🧠</text></g>${conceptSceneConnection(395, 150, 465, 150)}${conceptScenePanel(470, 75, 125, 150, conceptPart(parts, 2, "label", "Predicție"), "ieșire", active === 2)}${conceptScenePacket("M 200 150 L 465 150")}`;
  } else if (kind === "weather") {
    body = `${conceptSceneNode(100, 100, parts[0] || conceptVisualParts("🌡️ Senzor"), active === 0)}${conceptSceneConnection(145, 100, 275, 100)}${conceptScenePanel(280, 55, 155, 90, conceptPart(parts, 1, "label", "JSON"), "mesaj", active === 1)}${conceptSceneConnection(435, 100, 495, 190)}<g class="weather-chart ${active === 2 ? "is-active" : ""}" transform="translate(500,190)"><rect x="-65" y="-45" width="130" height="90" rx="10"></rect><path class="chart-line" d="M -50 25 L -25 5 L 0 18 L 25 -18 L 50 -28"></path></g>${conceptScenePacket("M 145 100 L 495 190")}`;
  } else if (kind === "smart") {
    ...
    body = `${conceptScenePanel(55, 70, 130, 160, conceptPart(parts, 0, "label", "Imagine"), "📷", active === 0)}${conceptSceneConnection(185, 150, 255, 150)}<g class="vision-scan ${active === 1 ? "is-active" : ""}" transform="translate(320,150)"><rect x="-48" y="-50" width="96" height="100" rx="12"></rect><path class="scan-line" d="M -38 -30 H 38 M -38 0 H 38 M -38 30 H 38"></path></g>${conceptSceneConnection(368, 150, 455, 150)}<g class="vision-bars ${active === 2 ? "is-active" : ""}" transform="translate(510,150)"><rect x="-55" y="18" width="22" height="42"></rect><rect x="-22" y="-5" width="22" height="65"></rect><rect x="11" y="28" width="22" height="32"></rect><text class="vision-score" x="0" y="-20">scor</text></g>${conceptScenePacket("M 185 150 L 455 150")}`;
  } else if (kind === "virtual") {
    body = `${conceptScenePanel(55, 85, 130, 130, conceptPart(parts, 0, "label", "Gazdă"), "PC", active === 0)}${conceptSceneConnection(185, 150, 255, 150)}<g class="virtual-machine ${active === 1 ? "is-active" : ""}" transform="translate(320,150)"><rect x="-50" y="-42" width="100" height="84" rx="12"></rect><rect x="-35" y="-25" width="70" height="18" rx="5"></rect><rect x="-35" y="0" width="70" height="18" rx="5"></rect></g>${conceptSceneConnection(370, 150, 455, 150)}${conceptScenePanel(460, 85, 130, 130, conceptPart(parts, 2, "label", "Snapshot"), "revin", active === 2)}${conceptScenePacket("M 185 150 L 455 150")}`;
    ...
    body = `${conceptScenePanel(70, 90, 135, 120, conceptPart(parts, 0, "label", "scor=0"), "cutie", active === 0)}${conceptSceneConnection(205, 150, 275, 150)}<g class="variable-op" transform="translate(320,150)"><circle class="scene-node-circle" r="32"></circle><text class="scene-panel-title" x="0" y="8">+10</text></g>${conceptSceneConnection(365, 150, 435, 150)}${conceptScenePanel(440, 90, 135, 120, conceptPart(parts, 2, "label", "scor=10"), "nou", active === 2)}${conceptScenePacket("M 205 150 L 435 150")}`;
  ...
  return conceptSceneFrame(scene, kind, uid, body, descriptionId);
}
...
function renderVisualStory(visual) {
  ...
  return `<div class="individual-scene" aria-label="${esc(visual.title)}">
    ...
    <div class="scene-stage" data-demo-art>${renderConceptScene(visual, 0, undefined, uid, `${uid}-story`)}</div>
    <p class="scene-story" id="${uid}-story">${esc(visual.story)}</p>
  </div>`;
}
function renderClassifyExample(demo, visual) {
  const sample = demo.items && demo.items[0];
  if (!sample || !Array.isArray(demo.zones) || !demo.zones[sample.zone]) return renderVisualStory(visual);
  const zoneLabel = demo.zones[sample.zone];
  return `<div class="individual-scene" aria-label="Exemplu rezolvat">
    <div class="scene-heading"><span class="scene-spark">✦</span><strong>Așa se joacă</strong><span class="scene-hint">un exemplu deja rezolvat</span></div>
    <div class="classify-example-row">
      <span class="classify-example-item">${esc(sample.label)}</span>
      <span class="classify-example-arrow" aria-hidden="true">➜</span>
      <span class="classify-example-zone">${esc(zoneLabel)}</span>
    </div>
    <p class="scene-story">Apasă un obiect, apoi apasă cutia potrivită — exact așa cum am pus „${esc(sample.label)}” la „${esc(zoneLabel)}”.</p>
  </div>`;
}
function renderQuizIntro(visual) {
  return `<div class="individual-scene" aria-label="${esc(visual.title)}">
    <div class="scene-heading"><span class="scene-spark">✦</span><strong>${esc(visual.title)}</strong><span class="scene-hint">gândește înainte să alegi</span></div>
